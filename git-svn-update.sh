...
do
  ...
  if [[ ! -e "$destination/$name.git" ]]; then
    echo "$(date) - Repository \"$destination/$name.git\" is not found." | tee -a "$log_file";
    continue;
  fi
  ...
  # Fetch updates.
  echo "$(date) - Fetching updates in \"$name\" repository..." | tee -a "$log_file";
  git svn fetch --log-window-size=10000 -A "$authors_file" --authors-prog="$dir/svn-lookup-author.sh" --quiet | tee -a "$log_file";
  git svn rebase --local | tee -a "$log_file";
  ...
  echo "$(date) - Update completed at $(date)." | tee -a "$log_file";
done < "$url_file"
